...
Анимация — **презентационный слой**. Алгоритм UI:

| candidates.size | Поведение UI |
|-----------------|--------------|
| 0 | Не запускать колесо; empty state |
| 1 | Короткая анимация «пульс» или 1 сегмент |
| 2–8 | Сегмент колеса = одно желание (название укорочено) |
| 9+ | На колесе показывать **до 8** случайных сегментов + гарантия, что **победитель** среди них; либо абстрактные сектора без подписей, финал — карточка с полным названием |

**Рекомендация MVP:** при `size > 8` крутить декоративное колесо без подписей всех позиций; итог всегда на **отдельной карточке** с полным `title`.
